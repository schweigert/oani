/**
   \file BRep.cpp
   \brief Implementacao dos metodos da classe BRep
...
BRep::BRep( BRep* ptrClone ):TexturedEntity( ptrClone )
{
	...
	for( vector<BRepGroup*>::iterator ptrGroup = ptrClone->listOfGroups.begin(); ptrGroup != ptrClone->listOfGroups.end(); ptrGroup++ )
		this->addGroup( new BRepGroup( (* ptrGroup ) ) );
...
bool BRep::LoadfromFile( string strFileName )
{
	...
	int     flag;                    // armazena o resultado de cada operaçao de leitura
	...
	char    strSection[100];         // ID da seçao sendo lida: g, v, vt, vn, usemtl, s ou f
	...
	do
	{
		flag = fscanf( ptrFileOBJ, "%s", &strSection );
		if( flag < 1 ) break;
		...
		else if( strcmp( strSection, "s" ) == 0 || strcmp( strSection, "#" ) == 0 ) // não esta claro o que a seçao "s" faz mas aparentemente nao faz falta :-P
		{
			char c;
...
bool BRep::LoadMaterialLibrary( string strPath, string strFileName )
{
	...
	char strSection[100];  // ID da seçao sendo lida: newmtl, Ns, Ka, Kd, Ks, Ni, d, illum ou map_Kd
	char strCurrentMaterial[100];  // ID do material atualmente selecionado
	...
	do
	{
		flag = fscanf( ptrFileMTL, "%s", &strSection );
		if( flag < 1 ) break;
		...
		else if( strcmp( strSection, "Ni" ) == 0 || strcmp( strSection, "d" ) == 0 || strcmp( strSection, "illum" ) == 0 ) // seçoes que serao desconsideradas
		{
			flag = fscanf( ptrFileMTL, " %f\n", &fValue );
			if( flag < 1 ) break;
		}
