/**
   \file Camera.cpp
   \brief Implementacao dos metodos da classe Camera
...
{
	this->vTranslate = (( Vector3 ) Vector3::UNIT_Z) * -1; // transformar um "const Vector3" em um "Vector3" a fim de permitir sua manipulação
	this->vFocalPoint = Vector3::ZERO;
	this->vUpVector = Vector3::UNIT_Y;
	this->cameraType = camType;
	this->setViewport( 800, 600 );
	this->setClippingPlanes( 1.0f, 150.0f );  // default para camera perspectiva
...
void Camera::Yaw( float fAngle )
{
	...
	this->vFocalPoint =	Vector3(   // calcula a rotação do ponto focal da camera no eixo Y
		temp.x * cos( fAngle ) + temp.z * sin( fAngle ),
		temp.y,
		temp.z * cos( fAngle ) - temp.x * sin( fAngle ) );

	this->vFocalPoint = this->vFocalPoint + this->vTranslate; // devolve o ponto focal da camera para sua posição original em coordenadas globais
}
/**
...
void Camera::Roll( float fAngle )
{
	...
	// como o up vector apenas denota direção e não posição, sua localização espacial é irrelevante, portanto não se faz necessaria a conversão entre sistemas de coordenadas
	// global <-> local
	Vector3 temp = this->vUpVector;
	this->vUpVector = Vector3(   // calcula a rotação do vetor orientação da camera no eixo X
...
void Camera::Pitch( float fAngle )
{
	...
	this->vFocalPoint =	Vector3(   // calcula a rotação do ponto focal da camera no eixo X
		temp.x ,
		temp.y * (float)cos( fAngle ) - temp.z * (float)sin( fAngle ),
		temp.y * (float)sin( fAngle ) + temp.z * (float)cos( fAngle ) );
	this->vFocalPoint = this->vFocalPoint + this->vTranslate; // devolve a posição da camera para sua posição original em coordenadas globais
...
void Camera::TurnAround_H( float fAngle )
{
	...
	this->vTranslate =	Vector3(   // calcula a rotação do ponto focal da camera no eixo Y
		temp.x * cos( fAngle ) + temp.z * sin( fAngle ),
		temp.y,
		temp.z * cos( fAngle ) - temp.x * sin( fAngle ) );

	this->vTranslate = this->vFocalPoint + this->vTranslate; // devolve o ponto focal da camera para sua posição original em coordenadas globais
}
/**
...
void Camera::TurnAround_V( float fAngle )
{
	...
	this->vTranslate =	Vector3(   // calcula a rotação do ponto focal da camera no eixo X
		temp.x ,
		temp.y * (float)cos( fAngle ) - temp.z * (float)sin( fAngle ),
		temp.y * (float)sin( fAngle ) + temp.z * (float)cos( fAngle ) );
	this->vTranslate = this->vFocalPoint + this->vTranslate; // devolve a posição da camera para sua posição original em coordenadas globais
