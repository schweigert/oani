...
void LinearInterpolator::OnLoop( double dAnimationTime )
{
    ...
    {
        // o tempo transcorrido na anima��o est� dentro do intervalo entre poses ?
            ...
            dAnimationTime <= this->listOfPoseAttr[ uiCounter+1 ]->getTime() )
        {
            ...
            AttrVector3 * ptrTweenAttr = new AttrVector3(
                                (( AttrVector3* )this->listOfPoseAttr[ uiCounter ])->getValue() * ( 1.0 - dFactor ) +
                                (( AttrVector3* )this->listOfPoseAttr[ uiCounter+1 ])->getValue() * dFactor, 0.0,
    ...
    //printf("LinearInterpolator::OnLoop()\n");
}
