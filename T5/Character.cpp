/**
   \file Character.cpp
   \brief Implementacao dos metodos da classe Character
...
void Character::setAction( string strActionName )
{
	for( vector<CharacterAction*>::iterator ptrAction = this->Repertoire.begin(); ptrAction != this->Repertoire.end(); ptrAction++ )
		if( (* ptrAction )->getActionName() == strActionName )
		{
			...
			// atualiza os objetos Keyframing para cada articulaçao do personagem
...
void Character::OnLoad( string strPath )
{
	...
	this->getChild( 0 )->attach( ptrLeftArmPivot ); // prende o braço esquerdo ao torso
	...
	ptrLeftArmPivot->getChild( 0 )->attach( ptrLeftHandPivot ); // prende a mao esquerda ao braço esquerdo
	...
	this->getChild( 0 )->attach( ptrRightArmPivot ); // prende o braço direito ao torso
	...
	ptrRightArmPivot->getChild( 0 )->attach( ptrRightHandPivot ); // prende a mao direita ao braço direito
